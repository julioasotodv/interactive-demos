...
async function startApplication() {
  ...
  const env_spec = ['https://cdn.holoviz.org/panel/0.14.4/dist/wheels/bokeh-2.4.3-py3-none-any.whl', 'https://cdn.holoviz.org/panel/0.14.4/dist/wheels/panel-0.14.4-py3-none-any.whl', 'pyodide-http==0.1.0', 'holoviews==1.15.4', 'hvplot==0.8.2', 'numpy', 'pandas']
...
('watermark', '-v -p numpy,pandas,panel,holoviews,hvplot,bokeh,param,pyviz_comms')


# In[ ]:


def five_param_logistic(X, a, b, c, d, s):
    return c + (d - c) / (1 + np.exp(b*(X-a)))**s
...
s_slider = pn.widgets.FloatSlider(name='𝑠', 
                                  start=0, 
                                  end=5, 
                                  step=0.05,
                                  value=0.1)
...
def plot_logistic(x_range, a, b, c, d, s):
    X = np.linspace(x_range[0], x_range[1], 500)
    df = pd.DataFrame({"x": X})
    df["f(x)"] = five_param_logistic(X, a, b, c, d, s)
...
- $$a$$ shifts the function alongside $$x$$. If $$s=1$$, it is usually known as the <em>mid-point</em>: the $$x$$ value producing a response half-way between $$c$$ and $$d$$
...
- $$s$$ is the curve asymmetry. If $$s=1$$ the curve is symmetric, yielding the Four-Parameter Logistic function
""")
...
# Bug in panel>=1.2.2 breaks viewport with reactive functions
dash = pn.Row(pn.Spacer(height=10, sizing_mode="stretch_width"),
              pn.Column(pn.pane.Markdown("# The Five-Parameter Logistic",
